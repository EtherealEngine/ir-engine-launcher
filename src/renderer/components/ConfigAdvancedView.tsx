  ...
  return (
    <Box sx={sx}>
      <FormControlLabel
        ...
        label={Storage.FORCE_DB_REINIT.replaceAll('_', ' ')}
        sx={{ marginTop: 2, marginLeft: 0 }}
        control={<Switch defaultChecked={false} sx={{ marginLeft: 2 }} />}
        value={localConfigs[Storage.FORCE_DB_REINIT] === 'true'}
        onChange={(_event, checked) => onChange(Storage.FORCE_DB_REINIT, checked ? 'true' : 'false')}
      />
    </Box>
  )
}
